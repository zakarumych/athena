    ...
    T: Num,
{
    ...
    fn from(matrix: Matrix3<T>) -> Self {
        let a = matrix.arrays();
        let (x, y, z, w) = if a[0][0] + a[1][1] + a[2][2] > T::ZERO {
            let s = (a[0][0] + a[1][1] + a[2][2] + T::ONE).sqrt() * T::TWO;
            (
                (a[1][2] - a[2][1]) / s,
                (a[2][0] - a[0][2]) / s,
                (a[0][1] - a[1][0]) / s,
            ...
            let s = (T::ONE + a[0][0] - a[1][1] - a[2][2]).sqrt() * T::TWO;
            (
                s * T::HALF * T::HALF,
                (a[1][0] + a[0][1]) / s,
                (a[2][0] + a[0][2]) / s,
            ...
            let s = (T::ONE + a[1][1] - a[0][0] - a[2][2]).sqrt() * T::TWO;
            (
                (a[1][0] + a[0][1]) / s,
                s * T::HALF * T::HALF,
                (a[2][1] + a[1][2]) / s,
            ...
            let s = (T::ONE + a[2][2] - a[0][0] - a[1][1]).sqrt() * T::TWO;
            (
                (a[2][0] + a[0][2]) / s,
                (a[2][1] + a[1][2]) / s,
                s * T::HALF * T::HALF,
        ...
        let sin_half = (T::ONE - w * w).sqrt();
        if sin_half.abs() < T::EPSILON {
            Rotor3::IDENTITY
        } else {
            Rotor3::from_axis_angle(
                Point3::ideal(x / sin_half, y / sin_half, z / sin_half),
                w.acos() * T::TWO,
            )
        }
    }
...
///
/// The matrix is decomposed into its rotation block and translation column.
/// Fine for one-off or import-time conversions, but not recommended in a
/// per-frame hot loop.
    ...
    T: Num,
{
    ...
    fn from(matrix: Matrix4<T>) -> Self {
        let a = matrix.arrays();
        let rotor = Rotor3::from(Matrix3::from_column_arrays([
            [a[0][0], a[0][1], a[0][2]],
            [a[1][0], a[1][1], a[1][2]],
            [a[2][0], a[2][1], a[2][2]],
        ]));
        // The `Translator3 -> Motor3` conversion halves its Z displacement.
        // Compensate before composing the affine translation with rotation.
        let translator = Translator3::from_offset(a[3][0], a[3][1], a[3][2] * T::TWO);
        Motor3::from(translator) * Motor3::from(rotor)
    }
}
...
mod tests {
    ...
    fn rotor_matrix_round_trip() {
        let cases: [(f32, [f32; 3]); 6] = [
            (core::f32::consts::FRAC_PI_4, [1.0, 0.0, 0.0]),
            (core::f32::consts::FRAC_PI_2, [0.0, 1.0, 0.0]),
            (-core::f32::consts::FRAC_PI_6, [0.0, 0.0, 1.0]),
            (core::f32::consts::PI, [1.0, 0.0, 0.0]),
            (core::f32::consts::PI, [0.0, 1.0, 0.0]),
            (core::f32::consts::PI, [0.0, 0.0, 1.0]),
        ];
    ...
    fn motor_matrix_round_trip() {
        let rotor =
            Rotor3::from_axis_angle(Point3::at(0.0f32, 0.0, 1.0), core::f32::consts::FRAC_PI_2);
